...
class List 
{
  ...
  int Prec(char ch) 
    ...
    {
    	BigDecimal currentVal=BigDecimal.ONE;
        BigDecimal lastVal=currentVal.add(BigDecimal.ONE);
        BigDecimal xSquare=x.multiply(x);
    ...
    {
    	BigDecimal sum=asin(decPow(BigDecimal.valueOf(1).subtract(x.multiply(x)),BigDecimal.valueOf(0.5),fix),fix);
    	return sum;
    }
    BigDecimal atan(BigDecimal x,int fix)
    {
    	BigDecimal sum=asin(x.divide(decPow(BigDecimal.valueOf(1).add(x.multiply(x)),BigDecimal.valueOf(0.5),fix),fix,RoundingMode.HALF_UP),fix);
    	return sum;
    }
    BigDecimal asec(BigDecimal x,int fix)
    {
    	BigDecimal sum=acos(BigDecimal.valueOf(1).divide(x,fix,RoundingMode.HALF_UP),fix);
    	return sum;
    }
    BigDecimal acsc(BigDecimal x,int fix)
    {
    	BigDecimal sum=asin(BigDecimal.valueOf(1).divide(x,fix,RoundingMode.HALF_UP),fix);
    	return sum;
    }
    BigDecimal acot(BigDecimal x,int fix)
    {
    	BigDecimal sum=atan(BigDecimal.valueOf(1).divide(x,fix,RoundingMode.HALF_UP),fix);
    	return sum;
    }
    ...
    {         
        ...
        { 
            ...
            else if(c=='\u2202')
            {
                ...
                int eB=endBP(exp.substring(i));               
                String bracExp=exp.substring(sB,i+eB+1);            	
            	if(!bracExp.contains("\u207B"))//cannot differentiate trigo or inverse trigo
            	{           		
            ...
            else if(c=='\u222B')// Works for medium ranged numbers
            {  	                        	
            	int sB=exp.indexOf('(',i);                
                int eB=endBP(exp.substring(i));                
                String bracExp=exp.substring(sB,i+eB+1);            	
...
public class Calculator implements ActionListener{	
	...
	private void initialize() {
		frame = new JFrame();
		frame.setBackground(new Color(255, 140, 0));
		frame.getContentPane().setBackground(new Color(0, 0, 0));
		frame.setBounds(100, 100, 455, 498);
		...
		btnDiv.setBounds(215, 258, 71, 49);
		...
		btnMul.setBackground(new Color(255, 165, 0));
		btnMul.setBounds(215, 308, 71, 49);
		...
		btnSub.setBackground(new Color(255, 165, 0));
		btnSub.setBounds(215, 358, 71, 49);
		...
		btn0.setBackground(new Color(255, 140, 0));
		btn0.setBounds(71, 408, 71, 50);
		...
		btnEqual.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {			
		        ...
		        exp=ob.change(exp);		   		      
		        //Expression should not contain terms such as 8x. Use 8*x.
		        String ans1=ob.eP(ob.iTP(exp,-1),fix,rad);
		        /*Using try catch instead of if "Invalid expression" condition because if xxx is typed 
		        then answer saves it as xxx so when we convert it to BigDecimal here, it'll give an error. 
		        That's why we're using try and catch.*/
		        try {
		...
		btnEqual.setBounds(143, 408, 71, 50);
		...
		btnAdd.setBounds(215, 408, 71, 50);
		...
		btnLog.setBounds(287, 208, 76, 49);
		...
		btnSin.setBounds(287, 308, 76, 49);
		...
		btnCos.setBounds(287, 358, 76, 49);
		...
		btnTan.setBounds(287, 408, 76, 50);
		...
		btnDeg.setBounds(287, 258, 76, 49);
		...
		btnBack.setBounds(364, 108, 75, 49);
		...
		btnCsc.setBounds(364, 308, 75, 49);
		...
		btnSec.setBounds(364, 358, 75, 49);
		...
		btnCot.setBounds(364, 408, 75, 50);
		...
		btnInv.setBounds(364, 258, 75, 49);
		...
		btnAns.setBounds(215, 158, 71, 49);
